...
function checkAuth() {
  ...
  if (!auth.loggedIn) {
    ...
    // Restrict Recent Activity to staff
    const recentActivity = document.getElementById('recentActivityCard');
    const activityTab = document.querySelector('.dash-tab[data-target="recentActivityCard"]');
    if (recentActivity) recentActivity.style.display = isPrivileged ? 'block' : 'none';
    if (activityTab) activityTab.style.display = isPrivileged ? 'flex' : 'none';

    // Control tabs container: hide for students, allow CSS media queries for staff
    const dashTabs = document.getElementById('dashboardTabs');
    if (dashTabs) {
        dashTabs.style.display = isPrivileged ? '' : 'none';
    }

    // Adjust dashboard grid if hidden (for desktop layout)
    const dashboardGrid = document.getElementById('dashboardGrid');
    if (dashboardGrid) {
      dashboardGrid.style.gridTemplateColumns = isPrivileged ? '1fr 1fr' : '1fr';
    }
  }
}
